/***************************************************************************
    rs1decode.c  -  Reed-Solomon & Burst Error Protection
                             -------------------
...
/***********************************************************************
 *
 * Changed for decode only and much simpler command lines for use
 * in rxamadrm by Ties Bos - PA0MBO
 * Date Dec 20th 2011
 * **********************************************************************/
...
#define RSBEP_MAGIC_NUM "In�ʘ8��R7���S�"
...
int main(int argc,char *argv[])
{
  ...
  long verbose=0,decode=0,quiet=0;
  int rs_bsize=0,rs_dsize=0,bep_size=0;
  ...
  if (argc != 2)
  {
	  printf("syntax is %s <filename.rs2>  \n", argv[0]) ;
	  exit(-1) ;
  }
  ...
  if ((fpout=fopen("tmpout.dat", "wb"))==NULL)
  {
	  printf("Cannot open tempout.dat for output \n");
	  exit(1);
  }
   
   tr_buf=malloc(RS_BSIZE*MAX_EXPECTED_CHUNKS*sizeof(dtype)); 
   if (tr_buf == NULL)
   {
	  fprintf(stderr, "cannot malloc tr_buf \n");
	 exit(-1);
   } 
  ...
  decode = 1 ;  
	...
	rs_bsize=RS_BSIZE;rs_dsize=RS_DSIZE;
        ...
        while (!feof(fpin))
        {
                got=fread(tr_buf,1,bep_size*rs_bsize,fpin); /* read the encoded data */
				if (!got) break;
                if (got < (bep_size*rs_bsize))
                {
					for (k=got; k < bep_size*rs_bsize; k++)
						  tr_buf[k]=0;
					printf("added %ld zeros", bep_size*rs_bsize - got);
                }
  ...
  printf("after freeing buffers \n");
  /* now remove header and trailer from tmpout.dat */
  ...
  if ((fpin=fopen("tmpout.dat", "rb"))==NULL)
  {
	  printf("Cannot open tempout.dat for input \n");
	  exit(1);
  }
  ...
  {
	  fprintf(stderr, "problems with bep_size coded in file \n");
	  printf("bep_size %d coded size %d \n", bep_size, (int) tr_buf[1] + ((int) tr_buf[2])*256) ;
		  exit(-1);
  }
  ...
  printf(" na p = strtok \n");
  strcat(orgfilename, ".");
  printf(" na strcat orgfilename result is %s  codedfile ext is %s \n",orgfilename, coded_file_ext);
  strcat(orgfilename, coded_file_ext);
  printf(" na strcat \n");
  printf("newfilename: %s \n", orgfilename);

  if ((fpout=fopen(orgfilename, "wb"))==NULL)
  {
	  printf("Cannot open tempout.dat for output \n");
	  exit(1);
  }
